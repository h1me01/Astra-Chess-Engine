...
namespace Chess {
    ...
    void initRookAttacks() {
        for (Square s = a1; s <= h8; ++s) {
            U64 edges = ((MASK_RANK[FILE_A] | MASK_RANK[FILE_H]) & ~MASK_RANK[getSquareRank(s)]) |
                        ((MASK_FILE[FILE_A] | MASK_FILE[FILE_H]) & ~MASK_FILE[getSquareFile(s)]);
            ROOK_ATTACK_MASKS[s] = (MASK_RANK[getSquareRank(s)] ^ MASK_FILE[getSquareFile(s)]) & ~edges;
            ...
            do {
                ...
                ROOK_ATTACKS[s][index] = slidingAttacks(s, subset, MASK_FILE[getSquareFile(s)]) |
                                         slidingAttacks(s, subset, MASK_RANK[getSquareRank(s)]);
    ...
    void initBishopAttacks() {
        for (Square s = a1; s <= h8; ++s) {
            U64 edges = ((MASK_RANK[FILE_A] | MASK_RANK[FILE_H]) & ~MASK_RANK[getSquareRank(s)]) |
                        ((MASK_FILE[FILE_A] | MASK_FILE[FILE_H]) & ~MASK_FILE[getSquareFile(s)]);
            BISHOP_ATTACK_MASKS[s] = (MASK_DIAGONAL[getSquareDiag(s)] ^ MASK_ANTI_DIAGONAL[getSquareAntiDiag(s)]) & ~edges;
            ...
            do {
                ...
                BISHOP_ATTACKS[s][index] = slidingAttacks(s, subset, MASK_DIAGONAL[getSquareDiag(s)]) |
                                           slidingAttacks(s, subset, MASK_ANTI_DIAGONAL[getSquareAntiDiag(s)]);
    ...
    void initLookUpTables() {
        ...
        for (Square s1 = a1; s1 <= h8; ++s1) {
            for (Square s2 = a1; s2 <= h8; ++s2) {
                ...
                if (getSquareFile(s1) == getSquareFile(s2) || getSquareRank(s1) == getSquareRank(s2)) {
                ...
                } else if (getSquareDiag(s1) == getSquareDiag(s2) || getSquareAntiDiag(s1) == getSquareAntiDiag(s2)) {
