...
INCBIN(Eval, "C:/Users/semio/Documents/Coding/Projects/Astra-Chess-Engine/src/eval/nn-7e82d78ef76a870-450.nnue");
...
namespace NNUE {
    ...
    int32_t output(const NNUE::accumulator &accumulator, Color side) {
        ...
        for (int chunks = 0; chunks < N_HIDDEN_SIZE / 256; chunks++) {
            ...
            for (int i = 0; i < 256; i++) {
                output += relu(accumulator[static_cast<int>(side)][i + offset]) * HIDDEN_WEIGHTS[i + offset];
            }

            for (int i = 0; i < 256; i++) {
                output += relu(accumulator[static_cast<int>(~side)][i + offset]) * HIDDEN_WEIGHTS[N_HIDDEN_SIZE + i + offset];
            }
